...
describe(`Stepper (${Platform.OS})`, () => {
  it('renders the label, value and native control', async () => {
    await render(<Stepper label="Quantity" value={3} onValueChange={jest.fn()} testID="st"/>, options);
    if (isIOS) {
  ...
  it('forwards step, min and max', async () => {
    await render(<Stepper value={16} step={2} min={10} max={32} onValueChange={jest.fn()} testID="st"/>, options);
    if (isIOS) {
      const {props} = screen.getByTestId('st');
  ...
  it('formats the displayed value', async () => {
    await render(<Stepper value={150} formatValue={v => `${v}%`} onValueChange={jest.fn()} testID="st"/>, options);
  ...
  it('tints the buttons with the accent seed', async () => {
    await render(
      <AccentProvider seed="#8959EA">
        <Stepper value={1} onValueChange={jest.fn()} testID="st"/>
      </AccentProvider>,
      options,
    );
    if (isIOS) {
  ...
  it('disables the control', async () => {
    await render(<Stepper label="Guests" value={2} onValueChange={jest.fn()} disabled testID="st"/>, options);
    if (isIOS) {
  ...
  (isIOS ? it.skip : it)('disables only the button at the reached bound', async () => {
    const {rerender} = await render(<Stepper value={1} min={1} max={5} onValueChange={jest.fn()}/>, options);
    expect(iconButton('Decrement').props.enabled).toBe(false);
    expect(iconButton('Increment').props.enabled).toBe(true);
    await rerender(<Stepper value={5} min={1} max={5} onValueChange={jest.fn()}/>);
    expect(iconButton('Decrement').props.enabled).toBe(true);
    expect(iconButton('Increment').props.enabled).toBe(false);
  });

  it('renders without a label', async () => {
    await render(<Stepper value={2} onValueChange={jest.fn()} testID="st"/>, options);
