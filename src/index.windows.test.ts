 ...
 * implementation (`NOT_ON_WINDOWS` in `vitest.config.mts`): a component that imports one
...
describe('expo-interface (windows)', () => {
  it('forbids a module without a Windows implementation', async () => {
    await expect(import('expo-web-browser')).rejects.toThrow('expo-web-browser has no Windows implementation');
    await expect(import('@expo/ui')).rejects.toThrow('@expo/ui has no Windows implementation');
  });
