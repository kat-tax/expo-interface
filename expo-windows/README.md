...
bundle, its assets and the pages of the app's DOM components (`'use dom'`,
under `www.bundle`) land in `windows/<App>/Bundle`, and the app loads
...
| `expo-image` | `Image` and `ImageBackground` over an image island — WinUI `Image`s on a canvas, fed by the system's codecs (JPEG, PNG, GIF, BMP, TIFF, WebP and HEIF as the machine has them), `SvgImageSource` for SVG, and a blurhash decoded by the runtime — with every `contentFit` and `contentPosition` laid out by the runtime, `placeholder` (a blurhash, or any source) shown until the picture is there, `transition` as a cross-dissolve of the duration and timing asked (the other effects dissolve too), `tintColor` painting the picture as a mask and `blurRadius` blurring its pixels, animated GIFs playing (`autoplay`, `startAnimating`, `stopAnimating`; WebP animates as far as the system's codec goes), `recyclingKey` remounting, the `onLoadStart`, `onProgress`, `onLoad`, `onError`, `onLoadEnd` and `onDisplay` events. Sources are `http(s)` URLs (a `User-Agent` is sent, and `headers` honoured), `file:` URIs and paths, `data:` URIs and bundled assets. The disk cache is `cache\ExpoImage` under the app's local data, keyed by `cacheKey` or the URL, written whole and shared with `Image.prefetch`, `getCachePathAsync`, `writeToCacheAsync`, `readFromCacheAsync` and `clearDiskCache`; the memory cache holds decoded pictures for the `memory` policies and `clearMemoryCache` empties it. `loadAsync` and `useImage` answer with an `ImageRef` (the cached file, its size and media type) that a view takes as its source; `generateBlurhashAsync` encodes one. A thumbhash is not decoded or made, SF Symbols are iOS's, and `priority`, `allowDownscaling`, `responsivePolicy`, `decodeFormat` and the live text are taken without effect. |
| `expo-glass-effect`, `expo-symbols` | Resolved to stand-ins: a plain view, nothing. |
...
| `@expo/dom-webview` | Expo's DOM components (`'use dom'`) render in a WinUI 3 `WebView2` hosted in a XAML island: the page gets `window.ReactNativeWebView` (`postMessage`, `injectedObjectJson`) before its own script, so marshalled props, native actions and `useDOMImperativeHandle` work as on iOS and Android. In development the page is Metro's; in a Release build `expo-windows bundle` exports each component's page into `Bundle\www.bundle` beside the exe, and the island serves that folder at a virtual host of its own (`https://expo-dom.bundle`, which the Metro transformer points `expo/dom` at), so a page has a secure origin rather than a `file:` one. Needs the WebView2 Runtime, which Windows 11 has. |
